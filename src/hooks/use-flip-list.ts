...
// and animates inside the same commit that moved the rows — before paint —
// so it cannot flash the final layout the way animation driven by another
// render cycle can. Purely presentational: it animates whatever the real
// layout did, so it can never disagree with the data. The transform is
// applied to the child's first element (the card), never to the measured
