...
//
// It stays this short because the store holds only the document. Selection,
// edit mode and layout live in ../ui-store, so a checkpoint cannot contain
// them, and undo has nothing to do but move.
...
// For a gesture that commits as it goes: the drag makes its own changes with
// the building blocks (moveTodo, reorderProjects) and seals them as one step
// when it lands. TinyBase ignores this when nothing changed, so a drag that
// went nowhere adds nothing to undo.
