...
export default function TaskListPane({
  ...
  const isPaneEditing = useIsPaneEditing(paneId, listId)
  const openTodoId = useOpenTodoId()
  // A list pane recedes while a task is being written in — the row's own
  // inline rename, or the editor that has taken the pane beside it. One tint
  // for both, so the two ways of editing a task read as one state.
  const isEditingTask = isPaneEditing || openTodoId !== undefined
  ...
  return (
    <section
      className={`flex h-full min-h-0 min-w-0 flex-col transition-colors duration-100 ${
        isEditingTask ? "bg-neutral-50" : "bg-white"
      }`}
    >
      <ContextMenu
        trigger={
