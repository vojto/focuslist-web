...
async function main() {
  ...
  await page.mouse.move(paneBox.x + paneBox.width / 2, paneBox.y + 30, { steps: 12 })
  ...
  await page.mouse.move(twoBox.x + twoBox.width / 2, twoBox.y + twoBox.height - 6, { steps: 8 })
  await page.waitForTimeout(150)
  ...
  await page.mouse.move(paneBox.x + paneBox.width / 2, paneBox.y + paneBox.height - 80, { steps: 8 })
  await page.waitForTimeout(150)
  order = await rowOrder(todayPane)
  ...
  const threeBox = await todayPane.locator("li", { hasText: "Three" }).boundingBox()
  await page.mouse.move(threeBox.x + threeBox.width / 2, threeBox.y + threeBox.height - 6, { steps: 10 })
  await page.waitForTimeout(150)
  await page.mouse.up()
  await page.waitForTimeout(200)
  order = await rowOrder(todayPane)
  check("same-list reorder One after Three", JSON.stringify(order) === JSON.stringify(["Dragged", "Two", "Three", "One", "Four"]))
