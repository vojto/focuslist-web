...
async function drag(page, from, to, { cancel = false } = {}) {
  ...
  await page.mouse.move(a.x + a.width / 2 + 12, a.y + a.height / 2 + 4, {
    steps: 4,
  })
...
async function main() {
  ...
  await drag(
    page,
    projectPane.getByText("Design homepage"),
    todayPane.getByText("Review PRs"),
  )
  await todayPane.getByText("Design homepage").waitFor({ timeout: 2000 })
  const badge = await todayPane
    .locator("li", { hasText: "Design homepage" })
    .getByText("Website")
    .count()
  ...
  await drag(
    page,
    projectPane.getByText("Write copy"),
    todayPane.getByText("Review PRs"),
    { cancel: true },
  )
  ...
  await drag(
    page,
    todayPane.getByText("Design homepage"),
    projectPane.locator("h1", { hasText: "Website" }),
  )
